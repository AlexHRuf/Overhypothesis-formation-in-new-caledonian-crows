# Switch prob generator with:
# - α updated by scaled replacement (rho_a)
# - β updated by EITHER global_counts OR alpha (toggle via beta_mode)
# - Per-ID lambda noise on log-scale
# - Centered trial effect on switch utility
# - Optional stickiness (constant switch cost)
generate_switch_data <- function(
    N_id = 16,
    N_board = 10,
    N_food = 2,
    seed = 12345,
    ...
    fixed_global_counts = c(60, 60),   # if NULL, use each ID's sampling totals
    
    # --- Sensitivity (lambda) and OH learning rates (rho) ---
    lambda_raw = 0.0,   # L = exp(lambda_raw); same for all IDs here
    rho_a = 0.20,       # OH alpha scaled-replacement rate in (0,1)
    rho_b = 0.20,       # OH beta  scaled-replacement rate in (0,1)
    
    # --- Fixed attractions on outcome scale (no learning here) ---
    fixed_A = c(0.9, 0.1),
    
    # --- Centered trial effect (added to switch utility only) ---
    trial_effect = 0.0,
    stickiness = 0.0,
    
    # --- Alpha/Beta update toggles ---
  ...
  for (i in seq_len(N_id)) {
    ...
    global_counts <- if (is.null(fixed_global_counts)) colSums(y_sampling[i,,]) else fixed_global_counts
    t_done <- 0
    ...
    for (t in 1:max_test_trials) {
      ...
      } else if (beta_mode == "global_counts") {
        global_counts <- global_counts + c(0,1)  # increment LV
  ...
  list(
    N_id = N_id, N_board = N_board, N_food = N_food,
    T_id = as.integer(T_id), T_max = as.integer(T_max),
    treatment = treatment,
    y_sampling = y_sampling,
    alpha_init = alpha0,
    beta_init  = beta0,
    lambda_raw = lambda_raw, rho_a = rho_a, rho_b = rho_b,
    fixed_A = fixed_A, trial_effect = trial_effect,
    break_on_switch = break_on_switch,
    alpha_mode = alpha_mode, alpha_decay = alpha_decay,
    beta_mode = beta_mode,
...
#Current stan:
#Alpha gets updates using counts, beta gets updated as normalize(alpha).
dataList <- generate_switch_data(N_id = 20 , 
                                 mixed_alpha_total = 20, 
                                 uniform_alpha_total = 1,
                                 fixed_global_counts = c(60,60),
                                 alpha_mode = "dirichlet", #"dirichlet" or "decay"
                                 alpha_decay = 0.05, #if alpha_mode = "decay"
...
dataList <- generate_switch_data(N_id = 20 , 
                                 ...
                                 alpha_decay = 0.05, #if alpha_mode = "decay"
                                 beta_mode = "alpha", #"global_counts" or "alpha"
                                 rho_a = 0, #if alpha_mode = "dirichlet"
                                 rho_b = 0, #if beta_mode = "alpha"
                                 lambda_raw = 3,
                                 trial_effect = 0,
                                 stickiness = 2,
                                 break_on_switch = F
...
#Alpha decay updating:
#Alpha decays over time towards baseline, beta is updated using normalize(global counts + test counts) 
#If alpha represents the strength with which ID believes that b11 is representative of beta, it makes sense for that belief to decay quickly when encountering an unexpected run of LV food (for mixed).
dataList <- generate_switch_data(N_id = 40 , 
                               mixed_alpha_total = 20, 
                               uniform_alpha_total = 1,
                               fixed_global_counts = c(60,60),
