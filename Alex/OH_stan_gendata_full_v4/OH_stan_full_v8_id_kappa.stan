// Combo model: M1 RE & decays + M2 attraction updating + per-ID kappa
...
parameters {
  ...
  real lambda_raw;                                 // utility sensitivity (mean)
  real phi_raw;                                    // attraction update rate (mean)
  real kappa_raw;                             // switch penalty (mean)
  ...
  // Individual random effects for (λ, φ, kappa) — 3-variate
  matrix[3, N_id] z_ID;                           // standard normal
  vector<lower=0>[3] sigma_ID;                    // scales
  cholesky_factor_corr[3] Rho_ID;                 // correlations
}

transformed parameters {
  // RE offsets: v_ID[j, 1]=λ offset, v_ID[j, 2]=φ offset, v_ID[j, 3]=kappa offset
  matrix[N_id, 3] v_ID = (diag_pre_multiply(sigma_ID, Rho_ID) * z_ID)';
  ...
  { // local (not saved)
    ...
    for (s in 1:N_test) {
      ...
      // Utilities and stay/switch probabilities (per-ID L and S)
      real L = exp(lambda_raw + v_ID[j, 1]);
      real S = exp(kappa_raw + v_ID[j, 3]);  // per-ID kappa (stickiness)
      real U_b11 = dot_product(P_F_b11, A_F);
      real U_b12 = dot_product(P_F_b12, A_F);

      vector[2] util;
      util[1] = L * U_b11;          // stay B11
      util[2] = L * U_b12 - S;      // switch B12 has penalty
...
model {
  ...
  // 3D random effects for (λ, φ, kappa)
  ...
  // Sensitivity / update / stickiness population means
