// Combo model: M1 RE & decays + M2 attraction updating & modern syntax
...
parameters {
  ...
  // Individual random effects for (λ, φ)  — 2-variate (Model 1)
...
transformed parameters {
  ...
  { // local (not saved)
    ...
    for (s in 1:N_test) {
      ...
      // Utilities and stay/switch probabilities (no stickiness)
      ...
      t_ctr[j] += 1;
      // α: total mass shrinks exponentially
      alpha_t[j] = (1.0 - d_alpha) * alpha_t[j];
      // β: HV share shrinks exponentially, keep simplex
      beta_t[j][1] = (1.0 - d_beta) * beta_t[j][1];
      ...
      // P in (0,1) with per-ID RE on φ
      real P = inv_logit(phi_raw + v_ID[j, 2]);
      for (f in 1:N_food) {
        if (y_test[s] == f) {
          // If eaten, nudge toward 0.25; else decay only
          if (y_test_ate[s] + 1 == f) {
...
model {
  ...
  // Sensitivity & attraction-update scales
