// Combo model: M1 RE & decays + M2 attraction updating + stickiness + Stan 2.34+
...
parameters {
  ...
  real stickiness_raw;                             // penalty against switching
...
transformed parameters {
  ...
  { // local (not saved)
    ...
    for (s in 1:N_test) {
      ...
      // Utilities and stay/switch probabilities (with stickiness penalty)
      real L = exp(lambda_raw + v_ID[j, 1]);
      real S = exp(stickiness_raw);              // global (no RE) inertia cost
...
model {
  ...
  // Sensitivity / update / stickiness scales
