data{
  ...
  real PT_log_dur[N_id];  //Total duration of preference test in seconds (log transformed)

  //Sampling variables
  int<lower=1> N_board;                   //Number of boards in the sampling phase (10)
  int y_sampling[N_id, N_board, N_food];  // Total counts of each food item per board per bird
  ...
  int<lower=0, upper=1> y_test_ate[N_test]; // Observed eating (0 = no, 1 = yes) of sample from board 11
...
parameters{
  ...
  simplex[N_food] theta[N_id, N_board]; //Local knowledge about food item variability & distribution
...
transformed parameters{
  ...
  { //Local scope i.e., not saved in output
    ...
    vector[N_food] alpha_s[N_id]; //Step-specific alpha for each individual
    vector[N_food] beta_s[N_id];  //Step-specific beta for each individual
    ...
    for(s in 1:N_test){
      ...
      vector[N_food] P_F_b11;   //Food prob for board 11
      vector[N_food] P_F_b12;   //Food prob for board 12
      ...
      new_alpha = alpha[id[s]] + to_vector(food_counts[id[s], :]);
      ...
      P_F_b11 = softmax(to_vector(food_counts[id[s], :]) + alpha_s[id[s]] .* beta_s[id[s]]); //Responds to local sample experience
      ...
      A_F = to_vector(ALL_F[id[s], ]); // Convert row to vector
      A[s, ] = to_row_vector(ALL_F[id[s], ]); // Assign to global matrix
...
model{
  ...
  for(j in 1:N_id){
    ...
    // Replace the vectorized assignment with an explicit loop
    for(f in 1:N_food){
      alpha[j, f] ~ exponential(gamma[j]); // Overhypothesis about food item variability
    }
